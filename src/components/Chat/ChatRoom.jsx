...
export default function ChatRoom({ room, myPlayer, session }) {
  ...
  async function handleAction(actionText) {
    ...
    try {
      ...
      if (!res.ok || data?.error) {
        gmError = data?.error || res.statusText;
      } else {
        gmResponse = data;
      }
    } catch (e) {
      gmError = e?.message || String(e);
    }
    ...
    const narrativeContent = gmResponse?.narrative
      ...
      : `The realm shifts...\n[GM_ERROR: ${gmError || 'Unknown error'}]`;
  ...
  async function processGMActions(actions, freshPlayers) {
    for (const action of actions) {
      ...
      } catch (e) { /* action processing failure is non-critical */ }
    }
  }
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: 'var(--bg-deep)' }}>
      ...
      >
        <div style={{ fontFamily: 'Cinzel, serif', fontSize: '14px', fontWeight: 700, color: 'var(--gold)' }}>
          {room.name}
        </div>
        <div
          style={{
      ...
      <div className="flex-1 overflow-y-auto" style={{ paddingTop: '8px', paddingBottom: '8px' }}>
        ...
        {gmProcessing && (
          <div style={{ padding: '14px 16px', background: 'var(--gm-bg)', borderLeft: '3px solid var(--ember)' }}>
